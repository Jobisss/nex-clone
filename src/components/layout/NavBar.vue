...
<template>
  <v-app-bar class="px-5" density="comfortable" flat floating height="89" scroll-behavior="fade-image">
    <template #prepend>
      <div class="mr-5">
        <v-app-bar-nav-icon v-show="$vuetify.display.xs" icon="mdi mdi-menu"></v-app-bar-nav-icon>
        ...
        <v-menu>
          ...
          <v-list nav>
            <v-list-item v-for="(category, index) in categories"
              :to="{ name: 'category', params: { category: category.href } }" :key="index">
              <v-list-item-title>{{ category.title }}</v-list-item-title>
            </v-list-item>
          </v-list>
        </v-menu>
        <v-btn class="text-uppercase" style="font-weight: 500; font-size: 14px; color: #f2af29" :to="{ name: 'intro' }">
          Produzir Eventos </v-btn>
      </div>
    </template>
    <template #append>
